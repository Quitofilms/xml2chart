...
# --- UI Header ---
st.title("Pro Chord Grid Web v1.9")
st.write("Upload a MusicXML file to generate a rhythmic chord chart.")

# --- Sidebar Settings ---
with st.sidebar:
    st.header("Settings")
    cols = st.slider("Measures per row", 4, 8, 4)
    start_bar = st.number_input("Start Measure", value=1)
    end_bar = st.number_input("End Measure (0 for all)", value=0)
    ...
    find_str = st.text_input("Find", value="-")
    replace_str = st.text_input("Replace", value="m")
...
def generate_pdf(xml_data):
    ...
    for m in part.findall("measure"):
        ...
        if harmony_tags:
            ...
            for h in harmony_tags:
                root_node, kind_node, offset, bass_node = h.find(".//root-step"), h.find("kind"), h.find("offset"), h.find("bass/bass-step")
                ...
                bass = bass_node.text if bass_node is not None else None
                ...
                if include_bass and bass: chord_text += f"/{bass}"
        ...
        if start_bar <= internal_count <= limit:
            measures.append({"num": str(internal_count), "chords": chords_in_m})
        internal_count += 1
    ...
    margin = 15*mm
    cw = (w - (2*margin)) / cols
    ch = 22*mm # Standard cell height
    curr_x, curr_y = margin, h - 45*mm

    c.setFont("Helvetica-Bold", 18)
    c.drawString(margin, h - 25*mm, song_title.upper())
    ...
    for i, m in enumerate(measures):
        ...
        if (i + 1) % cols == 0 or (i + 1) == len(measures):
            c.line(curr_x + cw, curr_y, curr_x + cw, curr_y + ch)
        c.line(curr_x, curr_y, curr_x + cw, curr_y)
...
if uploaded_file is not None:
    pdf_buf, name = generate_pdf(uploaded_file)
